...
void videoTask(void *arg)
{
    ...
    while(1)
    {
        ...
        write_sms_frame(param, palette, isGameGear);
...
static void SaveState()
{
    ...
    if (romName)
    {
        ...
        // if (r != ESP_OK)
        // {
        //     odroid_display_show_sderr(GAMEPAD_SD_ERR_NOCARD);
        //     abort();
        // }
...
void app_main(void)
{
    ...
    else
    {
        ...
        if (cartSize == 0)
        {
            odroid_display_show_sderr(GAMEPAD_SD_ERR_BADFILE);
            abort();
        }
...
// #endif
    ...
    while (true)
    {
        ...
        int smsButtons=0;
    	if (joystick.values[GAMEPAD_INPUT_UP]) smsButtons |= INPUT_UP;
    	if (joystick.values[GAMEPAD_INPUT_DOWN]) smsButtons |= INPUT_DOWN;
    	if (joystick.values[GAMEPAD_INPUT_LEFT]) smsButtons |= INPUT_LEFT;
    	if (joystick.values[GAMEPAD_INPUT_RIGHT]) smsButtons |= INPUT_RIGHT;
    	if (joystick.values[GAMEPAD_INPUT_A]) smsButtons |= INPUT_BUTTON2;
    	if (joystick.values[GAMEPAD_INPUT_B]) smsButtons |= INPUT_BUTTON1;

        int smsSystem=0;
    	if (joystick.values[GAMEPAD_INPUT_START]) smsSystem |= INPUT_START;
    	if (joystick.values[GAMEPAD_INPUT_SELECT]) smsSystem |= INPUT_PAUSE;
